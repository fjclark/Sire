...
void OpenMMFrEnergyST::initialise()
{
    ...
    custom_cartesian_pos_rest->addGlobalParameter("lamrest", 1);
    ...
    custom_cartesian_orient_rest->addGlobalParameter("lamrest", 1);
    ...
    if (UseCartesian_flag == true)
    {
        for (int i = 0; i < nmols; i++)
        {
            ...
            if (has_cartesian_pos_rest)
            {
                ...
                const auto r1 = cartesian_pos_prop.property(QString("r1")).asA<VariantProperty>().toInt();
                const auto r2 = cartesian_pos_prop.property(QString("r2")).asA<VariantProperty>().toInt();
                const auto r3 = cartesian_pos_prop.property(QString("r3")).asA<VariantProperty>().toInt();
                const auto l1 = cartesian_pos_prop.property(QString("l1")).asA<VariantProperty>().toInt();
                const auto xr = cartesian_pos_prop.property(QString("xr")).asA<VariantProperty>().toInt(); // Index of dummy atom to be placed along xr
                const auto yr = cartesian_pos_prop.property(QString("yr")).asA<VariantProperty>().toInt(); // Index of dummy atom to be placed along yr
                const auto zr = cartesian_pos_prop.property(QString("zr")).asA<VariantProperty>().toInt(); // Index of dummy atom to be placed along zr
                const auto k_xr_l1 = cartesian_pos_prop.property(QString("k_xr_l1")).asA<VariantProperty>().toDouble();
                const auto k_yr_l1 = cartesian_pos_prop.property(QString("k_yr_l1")).asA<VariantProperty>().toDouble();
                const auto k_zr_l1 = cartesian_pos_prop.property(QString("k_zr_l1")).asA<VariantProperty>().toDouble();
                const auto xr_l1_0 = cartesian_pos_prop.property(QString("xr_l1_0")).asA<VariantProperty>().toDouble();
                const auto yr_l1_0 = cartesian_pos_prop.property(QString("yr_l1_0")).asA<VariantProperty>().toDouble();
                const auto zr_l1_0 = cartesian_pos_prop.property(QString("zr_l1_0")).asA<VariantProperty>().toDouble();

                const auto openmmindex_r1 = AtomNumToOpenMMIndex[r1];
                const auto openmmindex_r2 = AtomNumToOpenMMIndex[r2];
                const auto openmmindex_r3 = AtomNumToOpenMMIndex[r3];
                const auto openmmindex_l1 = AtomNumToOpenMMIndex[l1];
                const auto openmmindex_xr = AtomNumToOpenMMIndex[xr];
                ...
                if (Debug)
                {
                    ...
                    qDebug() << "r2 = " << r2 << " openmmindex_r2 =" << openmmindex_r2;
                    qDebug() << "r3 = " << r3 << " openmmindex_r3 =" << openmmindex_r3;
                    qDebug() << "Dummy atoms:";
                    qDebug() << "xr = " << xr << " openmmindex_xr =" << openmmindex_xr;
                    qDebug() << "yr = " << yr << " openmmindex_yr =" << openmmindex_yr;
                    qDebug() << "zr = " << zr << " openmmindex_zr =" << openmmindex_zr;
